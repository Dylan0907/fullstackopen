...
describe("favorite blog", () => {
  test("of emtpty list is null", () => {
    const result = listHelper.favoriteblog(emptyBlogs);
    assert.deepStrictEqual(result, null);
  });

  test("when list has only one blog should return this one", () => {
    const result = listHelper.favoriteblog(listWithOneBlog);
    assert.deepStrictEqual(result, listWithOneBlog[0]);
  });

  test("of a bigger list is correct one", () => {
...
describe("author with more blogs", () => {
  ...
  test("of a bigger list return the correct author", () => {
